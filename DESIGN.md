...
Stripi's design language opens with the gradient mesh. A wide horizontal band of cool white, pastel cream, sherbet orange, warm orange, and pale blue occupies the upper third of nearly every marketing page. Type and product UI mockups float above it on `{colors.canvas}` (white), with the gradient acting as both decoration and visual anchor. The lower portion of the page returns to white, with feature explanations on `{colors.canvas-soft}` (a barely-tinted cool off-white) and dashboard product mockups composited as faux IDE/console panels in deep navy.

The color system has two primary roles. **Orange** (`{colors.primary}` — `#f97316`) is the brand's signature CTA color, used sparingly: one filled pill per band. **Deep navy** (`{colors.ink}` — `#0d253d`) is the universal body text color and the fill of dashboard mockups, the featured pricing tier, and the dark-app surfaces on the dashboard track. Red (`{colors.ruby}`) is reserved for validation and warning states. Pink or magenta should not be used as a brand accent.
...
**Key Characteristics:**
- Gradient-mesh backdrop on every marketing hero — cool white/cream/orange/pale-blue horizontally washed across the upper third of the page.
- Single-orange CTA hierarchy: filled `{colors.primary}` pill is the only filled button on marketing surfaces.
...
- Cream-band feature cards (`{colors.canvas-cream}`) introduce a warm interlude between orange/white sections without breaking the brand's chromatic logic.
...
### Brand & Accent
- **Orange** (`{colors.primary}` — `#f97316`): The brand's signature CTA color. Filled-pill button, link emphasis, gradient anchor.
- **Orange Deep** (`{colors.primary-deep}` — `#ea580c`): A deeper orange used in gradient mid-stops and as the press-state warmer alternative.
- **Orange Press** (`{colors.primary-press}` — `#c2410c`): Pressed-state lift of the primary.
- **Orange Soft** (`{colors.primary-soft}` — `#fb923c`): A lighter orange used in product-UI accents and chart highlights.
- **Orange Subdued** (`{colors.primary-bg-subdued-hover}` — `#fed7aa`): Pale orange fill used as soft tag background.
- **Brand Dark 900** (`{colors.brand-dark-900}` — `#10233a`): The deep navy used on featured dark surfaces and dashboard chrome.
- **Ruby** (`{colors.ruby}` — `#dc2626`): Validation and warning red; never a brand accent or button color.
- **Magenta** (`{colors.magenta}` — `#fb923c`): Legacy alias mapped to orange soft to avoid pink brand surfaces.
...
- **On Primary** (`{colors.on-primary}` — `#ffffff`): Text on orange / dark-navy surfaces.
...
- Background `{colors.canvas-cream}`, text `{colors.ink}`, padding `{spacing.xxl}`, rounded `{rounded.lg}`. Used to break up the orange / white rhythm with warmth.
...
**Gradient Mesh Backdrop** — cool white → pastel cream → sherbet orange → warm orange → pale blue stops blurred horizontally across the upper third of the page. Keep pink out of the mesh so the site reads orange/navy instead of rosy.
...
- Don't add new accent colors outside the documented gradient stops (cool white / cream / orange / pale blue).
- Don't use the orange `{colors.primary}` as a body-text color — it's a CTA and link color, not a type color at body size.
